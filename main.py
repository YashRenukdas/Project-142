...
def recommended_articles():
  ...
  all_reccomend = pd.DataFrame(columns = col_names)

  for liked_article in liked_article:
    output = get_recommendations(liked_article["original_title"])
    all_reccomend = all_reccomend.append(output)
  all_reccomend.drop_duplicates(subset = ["title"], inplace = True)

  reccomended_movies_data = []
  for index, row in all_reccomend.iterrows():
     _p = {
      "url": row['url'],
      "title": row['title'],
      "text": row['text'] or "NA",
     ...
     reccomended_movies_data.append(_p)
     return jsonify({
      "data":reccomended_movies_data,
      "status":"success"
    })
